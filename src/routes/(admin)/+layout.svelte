...
{#if data.user}
	<div class="h-screen overflow-hidden">
		...
		>
			<div class="flex h-full flex-col">
				<div class="flex items-center gap-3 border-b p-4">
					<div
						class="bg-primary text-primary-foreground flex h-9 w-9 items-center justify-center rounded-lg"
					>
						<ShieldCheck class="h-5 w-5" />
					</div>
					<div>
						<h1 class="text-lg font-semibold">Admin Panel</h1>
						<p class="text-muted-foreground text-xs">Uppity Administration</p>
					</div>
				</div>
		...
		<div class="grid h-full grid-rows-[auto_1fr] lg:ml-64">
			...
			<main class="bg-background overflow-y-auto p-6">
				{@render children()}
			</main>
		</div>
	</div>
